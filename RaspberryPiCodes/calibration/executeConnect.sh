...
#/sbin/hwclock -s
PROJECTHOME=/home/pi/mbientlab/Project/calibration
cd $PROJECTHOME
DATAFILE1=data1
DATAFILE=data1.db
...
# diff2=`/usr/bin/expr $diffl / 10 `
 diff2=1
 blueLEDon
 /bin/echo $(/bin/date +"%D %T")"  python -u calibrationConnect.py $DEVICEADDR $diff2 >> $PROJECTHOME/excep.log" >> $PROJECTHOME/excep.log
 /usr/bin/python -u calibrationConnect.py $DEVICEADDR $diff2 >> $PROJECTHOME/excep.log
 result=$?
 offLED
 if [ $result -gt 0 ]; then
  flag=0
