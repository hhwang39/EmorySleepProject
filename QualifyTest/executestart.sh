...
string2=$(/bin/date -d "${INITDATE}+7 days" +"%D")" $INITTIME"
 ...
 blueLEDon
 /bin/echo $(/bin/date +"%D %T")"  python -u qualifyingTest.py $diff2 >> $PROJECTHOME/excep.log" >> $PROJECTHOME/excep.log
 /usr/bin/python -u qualifyingTest.py $diff2 >> $PROJECTHOME/excep.log
 result=$?
 offLED
 if [ $result -gt 0 ]; then
  flag=0
